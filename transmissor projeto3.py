...
def main():
    try:
        ...
        print(f'tamanho da imagem: {tamanho_img}')
        
        tamanho_loop = int(tamanho_img/50)
        bytes_enviados = 0

        resto_loop = tamanho_img % 50
        print(f'tamanho loop: {tamanho_loop}')


        quer_conectar = True
        esperando_handshake = True
        ...
        while quer_conectar:
            ...
            print()
            while esperando_handshake:
                timeout = 5  # Timeout de 5 segundos
                start_time = datetime.now()

                print('pegando pacote com handshake')
                dados, nRx = com1.getData(15)

                if dados == b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x10\x10\x10':
                ...
                if (datetime.now() - start_time).seconds >= timeout:
                    ...
                    break
                esperando_handshake = False

            if resposta == 'N' or resposta == 'n':
                print('comunicação será encerrada')
                com1.disable()
                quer_conectar = False
            elif resposta == 's':
                break
        ...
        for i in range(tamanho_loop):
            ...
            print(f'enviando o pacote numero {i}')
            bytes_enviados += 50
            if tamanho_img-bytes_enviados>=50:
                tamanho_prox = 50
            else:
                tamanho_prox = tamanho_img - bytes_enviados
            print(f'Tamanho do proximo pacote: {tamanho_prox}')
            payload += actual_imgBytes
            enviar_pacote_cheio(image_bytes= actual_imgBytes, index= i, total_pacotes=tamanho_loop, tamanho_do_prox= tamanho_prox, com1= com1)
            ...
            dados, nRx = com1.getData(15+tamanho_prox)

            print(f'acknowladge recebido: {dados}')
            print('-----------------------------------------------------------------------------------------')
            if dados[:12] == b'\x01\x02\x03\x04\x05\x06\x07\x08\x09\x10\x11\x12\x10\x10\x10':
                print('pacote enviado com sucesso')
                
            else:
                print('deu ruim! enviando de novo')
                print(dados)
                enviar_pacote_cheio(image_bytes= actual_imgBytes, index= i, total_pacotes=tamanho_loop, tamanho_do_prox= tamanho_prox, com1= com1)
        
        actual_imgBytes = image_bytes[-resto_loop:]
        print('enviando ultimo pacote')
        print('--------------------------------------')
        time.sleep(0.2)
        enviar_ultimo_pacote(actual_imgBytes,resto_loop,com1)
        if dados == b'\x01\x02\x03\x04\x05\x06\x07\x08\x09\x10\x11\x12\x10\x10\x10':
                print('pacote enviado com sucesso')
        else:
            print('deu ruim! enviando de novo')
            enviar_ultimo_pacote(actual_imgBytes,resto_loop,com1)
